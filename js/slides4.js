(async function(){
    ...
    function attachNavigationEvents(){
        ...
        window.addEventListener('keydown', function(evt) {
            const page = document.querySelector('.current').classList.contains('page')
            let keyCode = evt.keyCode, isTextarea = evt.target.tagName === 'TEXTAREA'
    ...
    function showNextTab() {
        ...
        const containers = slideObject.slide.querySelectorAll('.container-code>div')
        let lastIntex = 0
    ...
    function resizeSlide(slideObject) {
        ...
        function moveHandler(evt) {
            ...
            renderer.style.width = bcrRenderer.width + bcrCode1.width - bcrCode2.width + 'px'
    ...
    function tearDown(index) {
        ...
        const containerRenderer = slideObject.slide.querySelector( '.container-renderer')
    ...
    function showLibraries(slideObject) {
        ...
        const globals = poppup.querySelectorAll('.library input.global')
        checkboxes.forEach((check, i) => {
            check.checked = slideObject.libraries[i].selected || slideObject.libraries[i].global
    ...
    function refreshLibraries(slideObject) {
        if (slideObject.libraries === undefined) {
            ...
            config.libraries.forEach(lib => {
                ...
                slideObject.libraries.push({selected: global, name, url, version, global})
            })
            if(!librariesLoaded) {
                config.libraries.forEach(lib => {
